...
if __name__ == "__main__":
    ...
    actions = {}
    with open('./testes/'+dataname+f'/case{CASE}.csv', newline='') as file:
            data = list(csv.reader(file))
    for i, name in enumerate(data[0]):
        actions[name.replace(" ","")] = i 
    ...
    env.reset(f"SM/{ENV_NAME}.xml", rewards=reward, stop_crit=1, last_action=itemgetter(*last_actions)(actions), products=10, probs=probabilities)
    ...
    writer = SummaryWriter(comment=f"/DQN_{ENV_NAME}:dataname={dataname}.{CASE}_n={N_STEPS}_lr={LEARNING_RATE}_gamma={GAMMA}_epsDecay={EPSILON_DECAY_LAST_STEP}")
